    ...
    def build_shift_dict(self, shift):
        ...
        for idx, letter in enumerate(lowercase):
            try:
                ...
                shifted[letter] = lowercase[start + shift]
                start += 1

        start = -shift
        for idx, letter in enumerate(uppercase):
            try:
                shifted[letter] = uppercase[idx + shift]
            except IndexError:
                shifted[letter] = uppercase[start + shift]
                start += 1
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(26):
            ...
            countValid = 0
            for word in x.split(' '):
                valid = is_word(self.valid_words, word)

                if valid:
                    countValid += 1
                if countValid >= prevBest:
                    prevBest = countValid
                    best = [i, self.apply_shift(i)]
